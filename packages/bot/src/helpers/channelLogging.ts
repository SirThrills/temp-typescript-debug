...
import { ChannelLogRecord, ChannelLogRecordType } from 'lib-types'

export const addChannelLogRecord = async (
    client: Client,
    ...
    const channel = guild.channels.cache.find(
        (channel) => channel.id === recordChannel
    )
...
const getChannel = async (
    ...
    const [rows, _fields] = await client.execute<any[]>(
        'SELECT * FROM `guild_channel_logs` WHERE `guild_id` = ? AND `channel_type` = ?',
        [record.guild, record.type]
    )
    await client.end()
