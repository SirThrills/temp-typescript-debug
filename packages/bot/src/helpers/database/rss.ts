...
export const getGuildRssFeeds = async (guildId: string) => {
    ...
    const [rows, _fields] = await client.query<any[]>(
        'SELECT * FROM `guild_forum_rss_urls` WHERE `guild_id` = ?',
        [guildId]
    )
    await client.end()
...
export const getGuildRssFeed = async (guildId: string, feedId: number) => {
    const client = (await mysqlClient()).getConn()
    const [rows, _fields] = await client.query<any[]>(
        'SELECT * FROM `guild_forum_rss_urls` WHERE `id` = ? AND `guild_id` = ?',
        [feedId, guildId]
    )
    await client.end()
...
export const getGuildRssFeedLastPost = async (feedId: number) => {
    const client = (await mysqlClient()).getConn()
    const [rows, _fields] = await client.query<any[]>(
        'SELECT * FROM `guild_forum_rss_posts` WHERE `forum_rss_id` = ? ORDER BY `id` DESC LIMIT 1',
        [feedId]
    )
    await client.end()
    if (rows == null || rows.length !== 1) {
...
export const getGuildRssPost = async (postId: number) => {
    const client = (await mysqlClient()).getConn()
    const [rows, _fields] = await client.query<any[]>(
        'SELECT * FROM `guild_forum_rss_posts` WHERE `id` = ?',
        [postId]
    )
    await client.end()
    if (rows == null || rows.length !== 1) {
...
export const getGuildRssQueue = async (guildId: string) => {
    const client = (await mysqlClient()).getConn()
    const [rows, _fields] = await client.query<any[]>(
        'SELECT * FROM `guild_forum_rss_post_queue` WHERE `guild_id` = ?',
        [guildId]
    )
    await client.end()
    if (rows == null || rows.length === 0) {
...
export const addGuildRssPost = async (
    forumRssId: number,
    postUrl: string,
    postId: string,
    ...
    await client.query<any[]>(
        'INSERT INTO `guild_forum_rss_posts` (`forum_rss_id`, `post_url`, `post_id`, `post_content`, `post_image`, `timestamp`) VALUES (?,?,?,?,?,?)',
        [forumRssId, postUrl, postId, postContent, postImage, pubDate]
    )
    await client.end()
}
...
export const addGuildRssPostQueue = async (postId: number, guildId: string) => {
    const client = (await mysqlClient()).getConn()
    await client.query<any[]>(
        'INSERT INTO `guild_forum_rss_post_queue` (`post_id`, `guild_id`) VALUES (?,?)',
        [postId, guildId]
    )
    await client.end()
}
...
export const removeGuildRssPostQueueItem = async (queueId: number) => {
    const client = (await mysqlClient()).getConn()
    await client.query<any[]>(
        'DELETE FROM `guild_forum_rss_post_queue` WHERE `id` = ?',
        [queueId]
    )
    await client.end()
}
...
export const removeGuildRssFeed = async (feedId: number) => {
    const client = (await mysqlClient()).getConn()
    await client.query<any[]>(
        'DELETE FROM `guild_forum_rss_urls` WHERE `id` = ?',
        [feedId]
    )
    await client.end()
}
